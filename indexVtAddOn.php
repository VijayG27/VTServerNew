...
{
$query = "SELECT * FROM VtAddOns WHERE SerialNo = '".$row["TallySerialNo"]."' AND CompanyName = '".$row["Company Name"]."'";
...
if ($result) {
  if (mysqli_num_rows($result) > 0) {
	...
	print $traiDays;
	print "***";*/
	$sql = "Update VtAddOns SET AccountID = '".$row["Acc ID"]."', ProdName = '".$row["TallyProdInfo"]."', ProdType = '".$row["TallyProd"]."', TSS = '".$dt."' , ProdInfo = '".$row["VTProdInfo"]."' , MobileNo = '".$row["MobileNo"]."', Counter = '".$row["Counts"]."' Where SerialNo = '".$row["TallySerialNo"]."' AND CompanyName ='".$row["Company Name"]."'";
	...
	if ($conn->query($sql) === TRUE) {
		...
		$sql = "INSERT INTO VtAddOns (`CompanyName`, `SerialNo`, `AccountID`, `ProdName`, `ProdType`, `TSS`, `ProdInfo`, `MobileNo`, `ExpDt`) VALUES ('".$row["Company Name"]."', '".$row["TallySerialNo"]."', '".$row["Acc ID"]."', '".$row["TallyProdInfo"]."', '".$row["TallyProd"]."', '".$dt."','".$row["VTProdInfo"]."', '".$row["MobileNo"]."', '".$ExpDate."')";
			if ($conn->query($sql) === TRUE) {
