			...
			if ($result->num_rows > 0) {
				...
				while($row = $result->fetch_assoc()) {
					...
					echo "
						...
						<td><a href='updateuserregisterVtAddOns.php?editSN={$row['SerialNo']}&editcmp=$cmpName&editAccId={$row['AccountID']}&editProdName={$row['ProdName']}&editProdType={$row['ProdType']}&editTSS={$row['TSS']}&editProdInfo={$row['ProdInfo']}&editMobNo={$row['MobileNo']}&editExpDt={$row['ExpDt']}&editTrial={$row['trialperiod']}&editCounter={$row['Counter']}&editStatus={$row['Status']}&editRemark={$row['Remark']}'>{$row['SerialNo']}</a></td>
						<td>{$row['AccountID']}</td>
						<td>{$row['ProdName']}</td>
						<td>{$row['ProdType']}</td>
						<td>{$row['TSS']}</td>
