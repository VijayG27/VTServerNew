  ...
  <meta charset="UTF-8">
    ...
    <?php 
            ...
            $updateProdInfo = mysqli_real_escape_string($conn,$_POST['ucmpprodInfo']);
						...
						//$update_query = "Update customerinfo SET ProdInfo = '".$updateProdInfo."', ExpDt = '".$updateExpDt."', trialperiod = '".$updateTrail."', Status = '".$updateStatus."', Remark = '".$updateRemark."' Where SerialNo='".$serial_no."' AND CompanyName = '".$updateCmp."'"
            $update_query = "Update customerinfo SET ExpDt = '".$updateExpDt."', trialperiod = '".$updateTrail."', Status = '".$updateStatus."', Remark = '".$updateRemark."' Where SerialNo='".$serial_no."' AND CompanyName = '".$updateCmp."'";
						if ($conn->query($update_query) === TRUE) {
							echo "Update record created successfully";
							//exit(header('location: updateuserregister.php'));
							//header('location: CustomorView.php?editSN='.$serial_no.'&editcmp='.$updateCmp.'&editAccId='.$updateAccId.'&editProdInfo='.$updateProdInfo.'&editMobNo='.$updateMobNo.'&editExpDt='.$updateExpDt.'&editTrial='.$updateTrail.'&editCounter='.$updateCount.'&editStatus='.$updateStatus.'&editRemark='.$updateRemark);
